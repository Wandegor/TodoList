...
1. [ ] Корректный запрос → `201 Created`. Возвращается созданная задача.
2. [ ] Некорректный JSON → `400 Bad Request`.
3. [ ] Пустой текст → `400 Bad Request`.
4. [ ] Текст из пробелов → `400 Bad Request`.
5. [ ] Текст длиной > 1000 символов → `400 Bad Request`.
...
1. [ ] Корректный запрос → `204 No Content` и `completed` становится `true`
2. [ ] Несуществующая задача → `404 Not Found`.
3. [ ] Некорректный идентификатор → `400 Bad Request`.

### DELETE `/tasks/{id}`

1. [ ] Корректный запрос → `204 No Content`. Задача удаляется из БД
2. [ ] Некорректный идентификатор → `400 Bad Request`.
3. [ ] Несуществующая задача → `404 Not Found`.
